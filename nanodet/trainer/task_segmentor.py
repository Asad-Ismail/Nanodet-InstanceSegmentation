...
class SegmentationTrainingTask(LightningModule):
    """
    ...
    def __init__(self, cfg, evaluator=None):
        super(SegmentationTrainingTask, self).__init__()
        self.cfg = cfg
    ...
    def predict(self, batch, batch_idx=None, dataloader_idx=None):
        ...
        preds,features = self.forward(batch["img"])
        ## can imporve we can get mask from forward also
        masks = self.model.head.masks_process(preds,features,batch)
        results = self.model.head.post_process(preds,masks, batch)
        return results

    def training_step(self, batch, batch_idx):
        batch = self._preprocess_batch_input(batch)
        preds,features,loss, loss_states = self.model.forward_train(batch)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.weight_averager is not None:
            preds,features,loss,loss_states = self.avg_model.forward_train(batch)
        else:
            preds,features,loss,loss_states = self.model.forward_train(batch)
        ...
        if batch_idx % self.cfg.log.interval == 0:
            ...
            self.logger.info(log_msg)
        masks = self.model.head.masks_process(preds,features,batch)
        dets = self.model.head.post_process(preds,masks,batch)
        return dets
